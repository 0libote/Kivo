...
Create an API key in [Google AI Studio](https://aistudio.google.com/app/apikey), open Kivo Settings, and save it under AI. The model picker lists the text models available to your key via Google's ListModels API (blocklist only — speech/audio, image, video, music, computer-use, and agent families are hidden), falling back to curated suggestions offline or without a key. Choose the model under AI → Model, then use Test connection to confirm your key can use it. Kivo checks the key with a single ListModels fetch, not a full generation, so it is fast and spends no quota — and it verifies your selected primary and backup models are actually available to the key, telling "pick another model" apart from "check your API key".

The native client defaults to `gemini-3.8-flash` with low thinking for latency-sensitive edits. Requests explicitly set `store: false`. Any well-formed `gemini-*` text model id works, so newest models keep working without a Kivo update — only speech/audio, image, video, music, computer-use, and agent families are blocked. Set an optional backup model in Settings → AI: if the primary hits its rate limit, Kivo retries once on the backup before reporting an error. Update the curated fallback, blocklist, or endpoint in `src-tauri/src/ai/mod.rs` (mirrored in `src/ai/models.ts`) and update fixtures at the same time.
