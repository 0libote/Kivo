...
export function HomeSection({ context, settings, onWriting, onDictation }: { readonly context: AppContext; readonly settings: AppSettings; readonly onWriting: () => void; readonly onDictation: () => void }) {
  ...
  return <section className="settings-content home-content">
    ...
    <div className="home-shortcuts" aria-label="Your shortcuts">
      <button className="home-shortcut" type="button" onClick={onDictation} aria-label="Change dictation shortcut"><Icon name="microphone" size={19} /><div><strong>Dictation</strong><span>Hold to speak, release to finish</span></div><div className="shortcut-summary">{formatShortcut(settings.dictationShortcut, context.platform).map((key, index) => <kbd key={index}>{key}</kbd>)}</div></button>
      <button className="home-shortcut" type="button" onClick={onWriting} aria-label="Change Writing Tools shortcut"><Icon name="pencil" size={19} /><div><strong>Writing Tools</strong><span>Select text, then press your shortcut</span></div><div className="shortcut-summary">{formatShortcut(settings.writingShortcut, context.platform).map((key, index) => <kbd key={index}>{key}</kbd>)}</div></button>
    </div>
