...
const textSelection = { hasSelection: true, applicationName: "Editor", canReplace: true, initialText: "A transcript describing the project." };
const linkSelection = { ...textSelection, initialText: "https://example.com/article" };
const emptySelection = { hasSelection: false, applicationName: "Editor", canReplace: false, initialText: "" };
...
async function installHarness(page: Page, context: SelectionContext) {
  ...
  await page.evaluate(async ({ context }) => {
    ...
    nativeBridge.runWritingAction = (request) => {
      ...
      return new Promise((resolve, reject) => window.summaryTest.pending.push({ resolve, reject }));
    };
...
for (const platform of ["macos", "windows"] as const) {
  for (const theme of ["light", "dark"] as const) {
    test(`${platform} ${theme}: selected text and links use the compact Flow surface`, async ({ browser }, testInfo) => {
      const context = await browser.newContext({ viewport: { width: 360, height: 420 }, userAgent: platform === "windows" ? "Windows" : "Macintosh" });
...
test("empty selections stop at guidance instead of becoming a general AI textbox", async ({ page }) => {
